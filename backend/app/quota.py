...
class QuotaManager:
    ...
    def __init__(self, max_tokens_per_day: int = 50000, max_requests_per_day: int = 100):
        ...
        self.daily_usage: Dict[str, Dict[str, Any]] = defaultdict(
            lambda: {
                "tokens": 0,
                "requests": 0,
                "reset_at": None,
    ...
    async def check_quota(self, ip: str) -> bool:
        ...
        if usage["reset_at"] is None or now > usage["reset_at"]:
            usage["tokens"] = 0
            usage["requests"] = 0
            usage["reset_at"] = now.replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(
                days=1
            )
        ...
        if usage["tokens"] >= self.max_tokens_per_day:
            logger.warning(
                f"Token quota exceeded for IP: {ip} ({usage['tokens']}/{self.max_tokens_per_day})"
            )
            return False

        if usage["requests"] >= self.max_requests_per_day:
            logger.warning(
                f"Request quota exceeded for IP: {ip} ({usage['requests']}/{self.max_requests_per_day})"
            )
            return False
...
def get_quota_manager() -> QuotaManager:
    ...
    if _quota_manager is None:
        ...
        _quota_manager = QuotaManager(
            max_tokens_per_day=max_tokens, max_requests_per_day=max_requests
        )
